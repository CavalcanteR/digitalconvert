...
var async = require('async'),
 ...
 var Docs = new GDClient('consumer key', 'consumer secret');


  console.log(Docs);

//create new gdata - old
...
exports.index = function (req, res, eventEmitter) {
  res.render('index', { title: 'Digital Gurusoft Convert' });
};
...
exports.upload = function (req, response, eventEmitter) {
  ...
  var now = new Date(), 
  currentDate = now.getFullYear().toString() + (now.getMonth()+1) + now.getDate(), 
  ...
  Docs.clientLogin(creds.login, creds.passwd, '', '', getSession);
